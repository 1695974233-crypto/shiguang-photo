...
export default function Home() {
  ...
  const [minimalLayoutMode] = useState<"original" | "enlarged">("original");
    ...
    <main>
      ...
      <section className="studio" aria-label="照片创作工作台">
        ...
        <div className="assistant-line">
          ...
          <p>{selectedScene?.id === "light-ink-wash" ? "默认在底部配英文标题和一句简短描述。你可以指定文案与位置，或填写“不要文字”。" : selectedScene?.id === "minimal-zine" ? "可以填写想放在图片里的文字，或告诉我希望怎么调整；不需要文字时，输入“不要文字”。" : "是否需要在图片里面加文字？请写下文字内容并确认位置，例如：在左上角添加“夏日散步”。"}</p>
        </div>
        ...
        <div className="generation-bar">
          <div>
            ...
            <span>{selectedScene?.id === "portrait-relight" ? "保持原图比例 · 像素级后期" : selectedScene?.id === "gathered-scenes" ? "保持原图比例 · 原像素固定纸裁" : selectedScene?.id === "minimal-zine" ? `极简 Zine · ${selectedRatio} · ${instruction.trim() ? textPosition : "自动配文"}` : `${selectedRatio} · ${textPosition}`}</span>
          </div>
