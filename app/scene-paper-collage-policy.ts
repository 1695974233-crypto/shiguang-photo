...
export const scenePaperCollageSeparationContrast = `拾景纸刊必须在缩略图尺度一眼分清P与I。P是唯一的自然全彩摄影材料；I是暖象牙纸上由原图背景蒸馏出的低对比版画域，不是原背景的水彩滤镜、淡化照片、半透明照片、连续全彩重绘或第二张近似摄影。I最多使用两种相容印刷语言，从粗网点、干刷丝网、石墨/炭笔拓印、褪色浮雕版和稀疏机械线中选择；删除大部分微小细节，以少量宽阔轮廓、表面节奏和断续色块表达原背景，整体饱和度、连续色阶、边缘清晰度和细节密度都必须显著低于P。暖纸是I的可见底色，允许大面积低墨量，但这些低墨区仍由原背景的构图位置、主色倾向、明暗和方向组织，不能成为无来源的第三块画板。撕口默认是一处围住主体关系域的闭合、不规则、非矩形摄影岛；除非原图主体本来被画面边缘裁断，否则不得让P触碰或占满两条以上成图边缘，也不得用一条从画面一端贯穿另一端的撕缝把整幅照片机械分半。撕边位于主体与普通背景之间，离主体留自然缓冲，但必须让主体明显处于摄影侧、绝大部分普通背景明显处于版画侧。`;
