...
export const scenePaperCollageFullPageTopology = `拾景纸刊只有两个内容域，必须铺满整个成图：P摄影主体域与I绘画背景域。用画布C表示整张成图，则P与I互斥且P∪I=C；P之外的每一个位置都属于I，不存在第三块未分配画板、独立空白区或等待填充的N域。M纸张与印刷材料只改变P和I的承载质感，不能独占一块内容区域。I可以在原图本来安静、明亮或低信息的位置使用接近纸色的低密度表达，但仍必须由原图背景的颜色、明暗、纹理、方向、轮廓或空间节奏决定，并与整幅背景构图连续；不能只是模型没有生成内容后剩下的默认白纸。撕边B是同一场景从自然摄影切换为纸上绘画的材质边界，不是把一张照片贴到另一张背景上：至少两处源图背景结构，或一处宽阔连续的背景表面，必须在B两侧保持方位、透视、方向、尺度与层级对应。`;

export const scenePaperCollageSeparationContrast = `拾景纸刊必须在缩略图尺度一眼分清P与I。P是唯一的自然全彩摄影材料；I是暖象牙纸上由原图背景蒸馏出的低对比版画域，不是原背景的水彩滤镜、淡化照片、半透明照片、连续全彩重绘或第二张近似摄影。I最多使用两种相容印刷语言，从粗网点、干刷丝网、石墨/炭笔拓印、褪色浮雕版和稀疏机械线中选择；删除大部分微小细节，以少量宽阔轮廓、表面节奏和断续色块表达原背景，整体饱和度、连续色阶、边缘清晰度和细节密度都必须显著低于P。暖纸是I的可见底色，允许大面积低墨量，但这些低墨区仍由原背景的构图位置、主色倾向、明暗和方向组织，不能成为无来源的第三块画板。撕口默认是一处围住主体关系域的闭合、不规则、非矩形摄影岛；它的全画布位置必须继承原图主体关系域的归一化坐标，从主体本体向必要接触物、承托物和少量关系环境自适应扩张，不得为了版式平衡把撕口或主体移向左上、中央或任何固定象限。主体可以位于撕口内部的偏左、偏右、偏上或偏下位置，不要求接近撕口视觉中心；四周缓冲可以不等宽，但必须由原图关系与空间分界决定。除非原图主体本来被画面边缘裁断，否则不得让P触碰或占满两条以上成图边缘，也不得用一条从画面一端贯穿另一端的撕缝把整幅照片机械分半。撕边位于主体与普通背景之间，离主体留自然缓冲，但必须让主体明显处于摄影侧、绝大部分普通背景明显处于版画侧。`;

export const scenePaperCollageLayerOntology = `${scenePaperCollageSeparationContrast}
拾景纸刊包含四类视觉成分，必须先分类再验收来源：
1. SCENE_ELEMENT 场景元素：属于被描绘世界的人、动物、植物、建筑、器物、地形、天空、水面、道路、栏杆、墙面、地面、光影边界等具有场景语义的实体、表面或结构。只有这一类需要与原图P域之外的背景逐项核对来源。
2. COLLAGE_MATERIAL 拼贴材料：承载画面的暖白或天然纸张基底、纸色、纸纤维、纸纹、低密度纸色区、撕口纤维边、干墨吸收、网点/丝网/石墨/拓印的印刷质感、轻微套色偏差和扫描颗粒。它们是产品规定的表现材料，不是原图中的场景对象，来源判定一律为 not_applicable；不得因为原照片里没有纸张或印刷纹理而判定新增对象。但材料也不能形成独立于I背景域的未分配空白画板。
...
分类依据是它在最终画面中的语义角色，而不是颜色、位置或绘画风格。比如“网点化的树”仍是 SCENE_ELEMENT 树；“网点印刷颗粒”才是 COLLAGE_MATERIAL。纸船、纸伞、报纸、书本、纸盒等若作为照片场景中的具体物体，仍属于 SCENE_ELEMENT，不能误当材料。低密度纸色区仍位于I背景域内，必须与原图背景证据和全幅构图发生关系，不能被解释为第三块空白内容域。`;
